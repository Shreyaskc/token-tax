...
def fig_top_languages(est: pd.DataFrame):
    mean_premium = est.groupby("language")["value"].mean().sort_values(ascending=False).head(15)
    names = [LANGUAGE_NAMES.get(c, c) for c in mean_premium.index]
    ...
    ax.set_xlabel("mean token premium vs. English (across 8 tokenizers)", color=INK_PRIMARY)
...
def main():
    ...
    print("wrote paper/figures/tokenizer_trend.pdf and paper/figures/top_languages.pdf")
