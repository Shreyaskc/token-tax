...
percentile bootstrap 95\% confidence interval (9{,}999 resamples, fixed
seed) over the 1{,}012 per-sentence ratios, computed by
\texttt{evalci.ci(method="bootstrap")} \citep{chandrahas2026evalci} -- not
reimplemented, per this project's cross-artifact reuse policy. The
domain-robustness check (Section~\ref{sec:domain-robustness}) uses 1{,}999
resamples per comparison and a fixed 35\% relative-difference threshold,
set before running the check, for flagging a divergent comparison.

\paragraph{Compute.} No model training or inference is performed anywhere
in this pipeline -- every measurement is string tokenization, not a forward
pass, so there is no parameter count or GPU budget to report. The full
sweep (8 tokenizers $\times$ 203 languages $\times$ 1{,}012 sentences, plus
the domain-robustness check and all bootstrap resampling) runs entirely on
CPU and completes in well under an hour on a personal laptop-class machine.
...
FLORES-200 (CC-BY-SA-4.0), OPUS-100 (license unspecified on its Hugging
Face card, varying by the underlying OPUS source corpus), or eBible
Corpus (CC-BY-4.0 or other per-translation terms, varying by contributing
translation) sentence
text, which remain under their original sources' own licenses; each
tokenizer vocabulary likewise remains under its own vendor's model license.
The released dataset itself is CC0, since it contains no source text to
which upstream license terms would propagate. We did not perform
additional screening of the source corpora for personally-identifying or
offensive content beyond each corpus's own curation process; this is a
limitation shared with any secondary use of these corpora, mitigated here
by the fact that our own outputs are aggregate statistics, not text. The
motivating concern is fairness:
