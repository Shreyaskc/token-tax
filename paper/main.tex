...
\begin{abstract}
The same sentence costs 1$\times$ tokens in English but often 5--12$\times$ in
...
\texttt{NousResearch/Meta-Llama-3-8B} mirror of the identical vocabulary;
and Mistral's tokenizer turned out not to be gated at all once tested, only
...
devtest set (matching their methodology), not a mean of per-sentence
ratios.} Table~\ref{tab:validation} shows agreement within 1.1\% relative
...
19$\times$, and even its most improved 2024-era vocabularies still charge
Georgian, Kannada, or Malayalam speakers roughly 2--3$\times$ English's rate
for identical content. What has changed is which vocabulary a researcher or
policymaker should cite a number from -- and that number will keep
