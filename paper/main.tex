...
premium ranges from 2.13$\times$ (Gemma-2) to 4.48$\times$ (GPT-2). Every
tokenizer released in 2024 shows a lower mean premium than GPT-2 (2019) or
GPT-4's cl100k\_base (2022) -- consistent with larger vocabularies and more
linguistically diverse pretraining data, though we do not claim to isolate
which factor drives it. A domain-robustness check against a second,
non-encyclopedic corpus (mixed-domain OPUS-100 text and/or a
Bible-translation corpus) finds the premium replicates within a median 14\%
across 264 comparisons for the 30 highest-premium languages -- but 9 of
those 30 have no modern, ungated parallel corpus available on Hugging Face
for a second domain at all, a scarcity that is itself part of the
token-tax story.
...
19$\times$. Even the two lowest-mean-premium 2024-era vocabularies
(Gemma-2, GPT-4o) still charge Georgian, Kannada, or Malayalam speakers
roughly 2--4$\times$ English's rate for identical content. A
same-generation vocabulary trained differently (Qwen2.5) charges those
same three languages 5--7$\times$: vocabulary design choices, not just
release date, drive the remaining gap. What has changed is which
vocabulary a researcher or policymaker should cite a number from -- and
that number will keep changing. \texttt{tokentax} is built to be re-run,
not re-derived: the
